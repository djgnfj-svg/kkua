    ...
    <div className="w-full h-screen bg-gradient-to-br from-blue-50 to-purple-50 overflow-hidden">
      <div className="h-1/5 flex flex-col items-center justify-center relative">
        ...
        <div className="absolute top-4 right-4 text-center">
          ...
          <div
            className={`text-3xl font-bold ${gameState.timeLeft <= 10 ? 'text-red-500 animate-pulse' : 'text-gray-800'}`}
          >
            {gameState.timeLeft}초
          </div>
          {gameState.timeLeft === 0 && (
            <div className="text-red-500 text-sm mt-1 animate-bounce">
              시간 초과!
            </div>
          )}
        </div>
        ...
        <div className="mb-8 text-center">
          ...
          {gameState?.currentWord && (
            <div className="text-lg text-gray-500 mt-2">
              이전 단어:{' '}
              <span className="font-semibold">{gameState.currentWord}</span>
            </div>
          )}
        </div>
