...
const GameTimer = ({ timeLeft, totalTime = 120, inputTimeLeft, inputTimeTotal = 12 }) => {
  // 전체 게임 시간 진행률 계산
    ...
    <div className="flex flex-col items-center space-y-4">
      ...
      <div className="relative">
        <svg className="w-32 h-32 transform -rotate-90">
          ...
          <circle
            ...
            r={radius}
            stroke={
              gameProgress > 50 ? 'rgb(34 197 94)' : 
              gameProgress > 25 ? 'rgb(250 204 21)' : 
              'rgb(239 68 68)'
            }
            strokeWidth="8"
            fill="none"
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className={`text-3xl font-bold ${getTimeColor(timeLeft, totalTime)}`}>
            {timeLeft}
          </div>
      ...
      <div className="w-full max-w-xs">
        <div className="flex justify-between items-center mb-1">
          ...
          <span className={`text-sm font-bold ${getTimeColor(inputTimeLeft, inputTimeTotal)}`}>
            {inputTimeLeft}초
          </span>
        </div>
        ...
        <div className="relative h-3 bg-gray-200 rounded-full overflow-hidden">
          ...
          <div
            className={`h-full transition-all duration-1000 ease-linear rounded-full ${
              inputProgress > 50 ? 'bg-green-500' :
              inputProgress > 25 ? 'bg-yellow-500' :
              'bg-red-500'
            }`}
