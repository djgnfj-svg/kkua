...
class GuestService:
        ...
        if guest_uuid:
            try:
                ...
                if not guest:
                    ...
                    guest = self.repository.create(
                        uuid_obj, nickname or default_nickname, device_info
                    )
            ...
            guest = self.repository.create(
                new_uuid, nickname or default_nickname, device_info
            )
    ...
    def _set_auth_cookies(self, response: Response, uuid_value: uuid.UUID):
        ...
        response.set_cookie(
            key=GUEST_AUTH_COOKIE, value=uuid_str, httponly=True, **cookie_options
        )

        # 2. 프론트엔드 식별용 쿠키 (JavaScript에서 접근 가능)
        response.set_cookie(
            key=GUEST_UUID_COOKIE, value=uuid_str, httponly=False, **cookie_options
        )
