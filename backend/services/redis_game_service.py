...
class RedisGameService:
    ...
    async def connect(self, max_retries: int = 3, retry_delay: float = 1.0):
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_exception = e
                logger.warning(
                    f"Redis 연결 실패 (시도 {attempt + 1}/{max_retries}): {e}"
                )
                ...
                last_exception = e
                logger.warning(
                    f"Redis 연결 타임아웃 (시도 {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2
                ...
                last_exception = e
                logger.error(
                    f"예상치 못한 Redis 연결 오류 (시도 {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2
    ...
    async def _smart_broadcast(
        self, room_id: int, message: dict, message_key: str = None
    ):
        ...
        try:
            ...
            if message_key != "game_time_update" and message_key.startswith(
                "game_time"
            ):
                ...
                if last_message and self._messages_equal(last_message, message):
                    logger.debug(
                        f"중복 메시지 생략: room_id={room_id}, type={message_key}"
                    )
                    return
            ...
            logger.debug(
                f"스마트 브로드캐스트 성공: room_id={room_id}, type={message_key}"
            )
    ...
    async def is_connected(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Unexpected error in Redis connection check: {e}", exc_info=True
            )
            return False
        ...
        try:
            ...
            if game_mode:
                default_settings = {
                    ...
                    "enable_advanced_scoring": game_mode.get(
            ...
            # 첫 번째 플레이어도 무작위 선택
            first_player_index = (
                random.randint(0, len(shuffled_participants) - 1)
                if shuffled_participants
                else 0
            )
            ...
            game_state = {
                ...
                "current_player_id": shuffled_participants[first_player_index][
                    "guest_id"
                ]
                if shuffled_participants
                else None,
                "first_player_nickname": shuffled_participants[first_player_index][
                    "nickname"
                ]
                if shuffled_participants
                else None,
            ...
            for participant in participants:
                ...
                await self.redis_client.setex(
                    f"game:{room_id}:player:{guest_id}", 86400, json.dumps(player_data)
                )
            ...
            logger.info(
                f"게임 생성: room_id={room_id}, participants={len(participants)}"
            )
            return True

        except (ConnectionError, TimeoutError) as e:
            logger.error(
                f"Redis 연결 오류로 게임 생성 실패: room_id={room_id}, error={e}"
            )
            return False
        except (ResponseError, RedisError) as e:
            logger.error(
                f"Redis 서버 오류로 게임 생성 실패: room_id={room_id}, error={e}"
            )
            return False
        except (ValueError, KeyError) as e:
            logger.error(
                f"잘못된 데이터로 게임 생성 실패: room_id={room_id}, error={e}"
            )
            return False
        except Exception as e:
            logger.error(
                f"예상치 못한 게임 생성 실패: room_id={room_id}, error={e}",
                exc_info=True,
            )
            return False
    ...
    async def start_game(self, room_id: int, first_word: str = "끝말잇기") -> bool:
        ...
        try:
            ...
            current_player_nickname = game_state.get(
                "first_player_nickname", "알 수 없음"
            )
            try:
                await self._smart_broadcast(
                    room_id,
                    {
                        ...
                        "participants_order": [
                            p["nickname"] for p in game_state["participants"]
                        ],
            ...
            logger.info(
                f"게임 시작: room_id={room_id}, first_word={first_word}, first_player={current_player_nickname}"
            )
            return True
    ...
    async def submit_word(
        self, room_id: int, guest_id: int, word: str
    ) -> Dict[str, Any]:
        ...
        while retry_count < max_retries:
            try:
                ...
                async with self.redis_client.pipeline(transaction=True) as pipe:
                    ...
                    for field, default_value in default_fields.items():
                        if field not in game_state:
                            ...
                            logger.debug(
                                f"게임 상태 누락된 필드 보완: {field}={default_value} (room_id={room_id})"
                            )
                    ...
                    game_state["current_player_id"] = participants[next_index][
                        "guest_id"
                    ]
                    game_state["current_player_nickname"] = participants[next_index][
                        "nickname"
                    ]
                    ...
                    # 턴 시간 초기화
                    game_state["turn_start_time"] = datetime.utcnow().isoformat()
                    game_state["time_left"] = game_state["game_settings"][
                        "turn_time_limit"
                    ]
                    game_state["updated_at"] = datetime.utcnow().isoformat()
                    ...
                    if player_data_str:
                        ...
                        default_data = self._get_default_player_data(
                            guest_id, player_data.get("nickname", "Unknown")
                        )
                        for field, default_value in default_data.items():
                            if field not in player_data:
                                player_data[field] = default_value
                                logger.debug(
                                    f"누락된 필드 보완: {field}={default_value} (room_id={room_id}, guest_id={guest_id})"
                                )
                    else:
                        # 플레이어 데이터가 없으면 기본 데이터로 초기화
                        logger.warning(
                            f"플레이어 데이터가 없어 새로 생성: room_id={room_id}, guest_id={guest_id}"
                        )
                        nickname = next(
                            (
                                p["nickname"]
                    ...
                    game_mode_multiplier = game_state["game_settings"].get(
                        "score_multiplier", 1.0
                    )
                    ...
                    if game_mode_multiplier != 1.0:
                        score_info["final_score"] = int(
                            score_info["final_score"] * game_mode_multiplier
                        )
                        score_info["mode_multiplier"] = game_mode_multiplier
                        score_info["total_multiplier"] = (
                            score_info["total_multiplier"] * game_mode_multiplier
                        )
                    ...
                    if (
                        player_data["fastest_response"] is None
                        or response_time < player_data["fastest_response"]
                    ):
                        player_data["fastest_response"] = response_time
                    if (
                        player_data["slowest_response"] is None
                        or response_time > player_data["slowest_response"]
                    ):
                    ...
                    logger.info(
                        f"단어 제출 성공 (트랜잭션): room_id={room_id}, guest_id={guest_id}, word={word}"
                    )
                    return result
                ...
                error_message = str(e)
                if (
                    "WATCH" in error_message
                    or "EXEC" in error_message
                    or "concurrent" in error_message.lower()
                ):
                    # 동시성 충돌로 인한 재시도
                    retry_count += 1
                    logger.warning(
                        f"동시성 충돌 감지 - 재시도 {retry_count}/{max_retries}: room_id={room_id}, guest_id={guest_id}"
                    )
                    if retry_count < max_retries:
                        ...
                        logger.error(
                            f"단어 제출 최대 재시도 초과: room_id={room_id}, guest_id={guest_id}"
                        )
                        return {
                            "success": False,
                            "message": "동시 접근으로 인해 단어 제출에 실패했습니다. 다시 시도해주세요.",
                        }
                else:
                    # 다른 종류의 오류
    ...
    async def _validate_word(self, game_state: Dict, word: str) -> Dict[str, Any]:
        ...
        if len(word) < min_length:
            return {
                "valid": False,
                "message": f"단어는 최소 {min_length}글자 이상이어야 합니다.",
            }

        if len(word) > max_length:
            return {
                "valid": False,
                "message": f"단어는 최대 {max_length}글자 이하여야 합니다.",
            }
        ...
        if game_state["last_character"] and word[0] != game_state["last_character"]:
            return {
                "valid": False,
                "message": f"'{game_state['last_character']}'로 시작하는 단어를 입력하세요.",
            }
        ...
        if special_rules.get("long_words_only") and len(word) < 5:
            return {
                "valid": False,
                "message": "마라톤 모드에서는 5글자 이상 단어만 사용할 수 있습니다.",
            }
    ...
    async def _update_game_stats(self, room_id: int, word_entry: Dict):
        ...
        try:
            ...
            if response_time > 0:
                if (
                    stats_data["fastest_response"] is None
                    or response_time < stats_data["fastest_response"]
                ):
                    stats_data["fastest_response"] = response_time
                if (
                    stats_data["slowest_response"] is None
                    or response_time > stats_data["slowest_response"]
                ):
            ...
            if not stats_data["shortest_word"] or len(word) < len(
                stats_data["shortest_word"]
            ):
    ...
    async def _update_player_stats(
        self, room_id: int, guest_id: int, word: str, response_time: float = 0.0
    ):
        """플레이어 통계 업데이트"""
        try:
            player_key = f"game:{room_id}:player:{guest_id}"
            ...
            logger.debug(
                f"플레이어 통계 업데이트: guest_id={guest_id}, score={player_data['score']}, words={player_data['words_submitted']}"
            )
    ...
    async def _check_game_over(self, game_state: Dict) -> Dict[str, Any]:
        ...
        active_players = [
            p for p in game_state.get("participants", []) if p.get("is_active", True)
        ]
        ...
        # 2. 게임 시간 초과 (최대 30분)
        game_duration = datetime.now() - datetime.fromisoformat(
            game_state.get("started_at", datetime.now().isoformat())
        )
        ...
        if (
            consecutive_skips >= len(active_players) * 2
        ):  # 모든 플레이어가 2회씩 넘긴 경우
            return {
                "game_over": True,
    ...
    async def _broadcast_time_update(self, room_id: int, time_left: int):
        ...
        # - 5초 이하: 중요 알림 포함
        should_broadcast = (
            time_left <= 10  # 10초 이하는 매초 (중요)
            or time_left in [15, 20, 25, 30]  # 5초 간격의 중요 순간들
            or (
                time_left >= 30 and time_left % 30 == 0
            )  # 30초 단위로만 (60초, 90초 등)
        )
    ...
    async def _reset_consecutive_success(self, room_id: int, guest_id: int):
        ...
        try:
            ...
            if player_data_str:
                ...
                player_data["updated_at"] = datetime.utcnow().isoformat()
                await self.redis_client.setex(
                    player_key, 86400, json.dumps(player_data)
                )
                logger.debug(
                    f"연속 성공 초기화: room_id={room_id}, guest_id={guest_id}"
                )
        except Exception as e:
    ...
    async def get_player_stats(self, room_id: int, guest_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            is_participant = any(
                p["guest_id"] == guest_id for p in game_state.get("participants", [])
            )
            if not is_participant:
                logger.warning(
                    f"플레이어가 게임 참가자가 아님: room_id={room_id}, guest_id={guest_id}"
                )
                return None
        ...
        try:
            ...
            for participant in game_state["participants"]:
                player_stats = await self.get_player_stats(
                    room_id, participant["guest_id"]
                )
                if player_stats:
                    # 평균 응답 시간 계산
                    if (
                        player_stats.get("words_submitted", 0) > 0
                        and player_stats.get("total_response_time", 0) > 0
                    ):
                        player_stats["average_response_time"] = round(
                            player_stats["total_response_time"]
                            / player_stats["words_submitted"],
                            2,
                        )
                    else:
                    ...
                    logger.warning(
                        f"플레이어 통계 없음 - 기본 데이터 생성: room_id={room_id}, guest_id={participant['guest_id']}"
                    )
                    default_stats = {
                        "guest_id": participant["guest_id"],
                        "nickname": participant["nickname"],
    ...
    async def get_game_stats(self, room_id: int) -> Dict:
        ...
        try:
            ...
            if stats_data_str:
                ...
                # 평균 응답 시간 계산
                if (
                    stats_data.get("total_words", 0) > 0
                    and stats_data.get("total_response_time", 0) > 0
                ):
    ...
    async def validate_player_can_join(
        ...
        try:
            ...
            if other_games:
                return (
                    False,
                    f"다른 게임({other_games[0]})에 이미 참여 중입니다. 먼저 해당 게임을 종료해주세요.",
                )
    ...
    async def end_game(self, room_id: int) -> bool:
        ...
        try:
            ...
            for participant in game_state.get("participants", []):
                player_games_key = (
                    f"{self.PLAYER_GAMES_PREFIX}{participant['guest_id']}"
                )
    ...
    async def cleanup_game(self, room_id: int):
        ...
        try:
            ...
            logger.info(
                f"게임 데이터 정리 완료: room_id={room_id}, 삭제된 키 수: {len(keys_to_delete)}"
            )
