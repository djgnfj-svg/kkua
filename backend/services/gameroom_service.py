...
class GameroomService:
    ...
    def __init__(self, db: Session):
        ...
        if (
            not hasattr(self.ws_manager.word_chain_engine, "db")
            or self.ws_manager.word_chain_engine.db is None
        ):
    ...
    async def join_gameroom(self, room_id: int, guest: Guest) -> JoinGameroomResponse:
        """
        게임룸에 참가합니다.
                        ...
                        self.create_background_task(
                            self.ws_manager.broadcast_room_update(
                                ...
                                "host_changed",
                                {
                                    ...
                                    "new_host_nickname": new_host.guest.nickname
                                    if new_host.guest
                                    else f"게스트_{new_host.guest_id}",
    ...
    async def start_game(self, room_id: int, guest: Guest) -> Dict[str, str]:
        ...
        # 게임 시작 가능 여부 확인
        can_start, error_message = self.game_state_service.can_start_game(
            room_id, guest.guest_id
        )
        if not can_start:
            raise HTTPException(
        ...
        try:
            ...
            logger.info(
                f"즉시 게임 시작 WebSocket 브로드캐스트 완료: room_id={room_id}"
            )
        except Exception as e:
        ...
        try:
            ...
            await redis_game.create_game(
                room_id, participant_data, game_settings, game_mode_config
            )
            await redis_game.start_game(room_id, "끝말잇기")

            logger.info(
                f"Redis 게임 초기화 완료: room_id={room_id}, participants={len(participant_data)}"
            )
    ...
    async def end_game(self, room_id: int, guest: Guest) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(f"게임 종료 처리 완료: room_id={room_id}")
    ...
    async def complete_game(self, room_id: int, guest: Guest) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(f"수동 게임 완료 처리 완료: room_id={room_id}")
    ...
    async def _delayed_cleanup(self, redis_game, room_id: int, delay_minutes: int = 30):
        ...
        try:
            ...
            await redis_game.cleanup_game(room_id)
            logger.info(
                f"Redis 게임 데이터 지연 정리 완료: room_id={room_id}, delay={delay_minutes}분"
            )
        except Exception as e:
    ...
    async def toggle_ready_status_with_ws(
    ...
    async def _get_saved_game_result(self, room_id: int):
        """Redis에서 저장된 게임 결과를 조회합니다."""
        try:
            from services.redis_game_service import get_redis_game_service

            redis_game = await get_redis_game_service()
            game_result = await redis_game.get_final_game_results(room_id)
            
            if game_result:
                logger.info("Redis에서 저장된 게임 결과 발견")
                return game_result
    ...
    async def _get_redis_game_result(self, room_id: int):
        ...
        try:
            ...
            logger.debug(
                f"Redis 데이터 조회 결과: game_state={bool(game_state)}, "
            ...
            if game_state and not all_player_stats:
                logger.warning(
                    f"게임 상태는 있지만 플레이어 통계가 없음: room_id={room_id}"
                )
                # 기본 플레이어 데이터 생성 시도
            ...
            if all_scores_zero and no_words and game_has_used_words:
                logger.warning(
                    f"게임 데이터가 아직 완전히 처리되지 않음: room_id={room_id}"
                )
        ...
        for word_entry in word_entries:
            used_words_data.append(
                WordChainEntry(
                    ...
                    timestamp=datetime.fromisoformat(word_entry["submitted_at"])
                    if word_entry.get("submitted_at")
                    else datetime.now(),
    ...
    def _build_game_result_response(
        self,
        room_id,
        game_state,
        players_data,
        ...
        return GameResultResponse(
            ...
            average_response_time=round(
                game_stats.get("average_response_time", 0.0), 1
            ),
            ...
            started_at=datetime.fromisoformat(game_state["created_at"])
            if game_state.get("created_at")
            else datetime.now(),
