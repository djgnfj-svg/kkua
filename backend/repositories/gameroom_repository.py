...
class GameroomRepository:
    ...
    def get_all(self) -> List[Gameroom]:
        rooms = (
            self.db.query(Gameroom).filter(Gameroom.status == GameStatus.WAITING).all()
        )
        return rooms
    ...
    def find_by_id(self, room_id: int) -> Optional[Gameroom]:
        ...
        try:
            ...
            if room:
                print(
                    f"게임룸 조회: ID={room.room_id}, 제목={room.title}, 상태={room.status}"
                )
                if isinstance(room.status, str):
                    print(f"상태 타입: 문자열, 값: {room.status}")
                else:
                    print(
                        f"상태 타입: Enum, 값: {room.status.value if hasattr(room.status, 'value') else room.status}"
                    )
            else:
    ...
    def find_active_by_creator(self, guest_id: int) -> Optional[Gameroom]:
        ...
        return (
            ...
            .filter(
                Gameroom.created_by == guest_id, Gameroom.status != GameStatus.FINISHED
            )
            .first()
        )
        ...
        try:
            ...
            participant = (
                ...
                .filter(
                    ...
                    GameroomParticipant.left_at.is_(
                        None
                    ),  # 중요: 나가지 않은 참가자만 확인
                )
                .first()
            ...
            if participant:
                print(
                    f"참가자 조회 결과: 참가자 발견! ID={participant.participant_id}, 상태={participant.status}"
                )
            else:
                # 디버깅용 - 나간 참가자가 있는지 확인
                left_participant = (
                ...
                if left_participant:
                    print(
                        f"참가자 조회 결과: 이전에 참가했으나 나간 상태. left_at={left_participant.left_at}"
                    )
                else:
                    print("참가자 조회 결과: 해당 방에 참가한 적 없음")
    ...
    def find_other_participation(
        self, guest_id: int, excluding_room_id: int
    ) -> Optional[GameroomParticipant]:
    ...
    def update_participant_status(self, room_id: int, participant_id: int, status: str):
        ...
        try:
            ...
            self.db.commit()
            print(
                f"참가자 상태 업데이트 완료: 참가자ID={participant_id}, 상태={status}"
            )
            return participant
        except Exception as e:
            self.db.rollback()
    ...
    def count_participants(self, room_id: int) -> int:
        ...
        try:
            count = (
                self.db.query(GameroomParticipant)
                .filter(GameroomParticipant.room_id == room_id)
                .count()
            )
    ...
    def delete_gameroom(self, room_id: int) -> bool:
        ...
        try:
            ...
            # 게임룸 상태를 DELETED로 변경
            room.status = (
                GameStatus.DELETED.value
                if isinstance(GameStatus.DELETED.value, str)
                else GameStatus.DELETED
            )
            room.updated_at = datetime.now()
    ...
    def update_participant_left(
        self, participant_id: int, left_at: datetime, status: str
    ) -> bool:
    ...
    def start_game(self, room_id: int) -> bool:
        ...
        try:
            ...
            room.status = (
                ...
                if isinstance(GameStatus.PLAYING.value, str)
    ...
    def check_all_ready(self, room_id: int) -> bool:
        ...
        try:
            ...
            all_ready = all(
                p.status == ParticipantStatus.READY.value for p in participants
            )
    ...
    def end_game(self, room_id: int) -> bool:
        ...
        try:
            ...
            room.status = (
                ...
                if isinstance(GameStatus.WAITING.value, str)
