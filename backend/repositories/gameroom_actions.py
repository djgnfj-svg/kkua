...
class GameroomActions:
    ...
    ) -> Tuple[Gameroom, GameroomParticipant]:
        """
        게임룸을 생성하고 방장을 추가합니다.
        """
        try:
    ...
    def toggle_ready_status(self, room_id: int, guest_id: int) -> Optional[str]:
        ...
        try:
            ...
            updated = self.gameroom_repo.update_participant_status(
                room_id, guest_id, new_status
            )
    ...
    def start_game(self, room_id: int, host_id: int) -> bool:
        ...
        try:
            ...
            all_ready = all(
                p.status == ParticipantStatus.READY or p.is_creator
