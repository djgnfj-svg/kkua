...
class GameLogRepository:
    ...
    def create_game_log(self, room_id: int, max_rounds: int) -> GameLog:
        """새 게임 로그를 생성합니다"""
        game_log = GameLog(
            room_id=room_id, max_rounds=max_rounds, started_at=datetime.utcnow()
        )
        self.db.add(game_log)
        self.db.commit()
        self.db.refresh(game_log)
    ...
    def find_game_log_by_room_id(self, room_id: int) -> Optional[GameLog]:
        ...
        return (
            ...
            .order_by(desc(GameLog.created_at))
            .first()
        )
    ...
    def get_word_entries_by_game_log(self, game_log_id: int) -> List[WordChainEntry]:
        ...
        return (
            self.db.query(WordChainEntry)
            .filter(WordChainEntry.game_log_id == game_log_id)
            .order_by(asc(WordChainEntry.turn_number))
            .all()
        )

    def create_player_stats(self, game_log_id: int, player_id: int) -> PlayerGameStats:
        """플레이어 게임 통계를 생성합니다"""
        stats = PlayerGameStats(game_log_id=game_log_id, player_id=player_id)
        self.db.add(stats)
        self.db.commit()
    ...
    def get_player_stats_by_game_log(self, game_log_id: int) -> List[PlayerGameStats]:
        ...
        return (
            ...
            .filter(PlayerGameStats.game_log_id == game_log_id)
            .order_by(asc(PlayerGameStats.rank))
            .all()
        )
    ...
    def calculate_final_rankings(self, game_log_id: int):
        ...
        stats_list = (
            self.db.query(PlayerGameStats)
            .filter(PlayerGameStats.game_log_id == game_log_id)
            .order_by(desc(PlayerGameStats.total_score))
            .all()
        )
    ...
    def _calculate_game_statistics(self, game_log: GameLog):
        ...
        if entries:
            ...
            if response_times:
                game_log.average_response_time = round(
    ...
    def find_recent_games_by_player(
