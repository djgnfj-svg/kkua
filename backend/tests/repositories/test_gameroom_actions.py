...
class TestGameroomActions:
    def test_create_gameroom(self, db_session: Session):
        ...
        # 게임룸 생성 데이터
        room_data = {"title": "액션 테스트 룸", "max_players": 6}
    ...
    def test_start_game(self, db_session: Session):
        ...
        # 참가자 상태 확인
        host_participant = actions.gameroom_repo.find_participant(
            room.room_id, host.guest_id
        )
        visitor_participant = actions.gameroom_repo.find_participant(
            room.room_id, visitor.guest_id
        )
