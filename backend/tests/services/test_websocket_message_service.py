...
class TestWebSocketMessageService:
    ...
    async def test_handle_chat_message(self, websocket_service, mock_ws_manager, sample_guest):
    ...
    async def test_handle_chat_message_no_nickname(self, websocket_service, mock_ws_manager, sample_guest):
    ...
    async def test_handle_ready_toggle_normal_user(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest, mock_participant):
        ...
        # Execute
        await websocket_service.handle_ready_toggle(mock_websocket, room_id, sample_guest)
        
        # Verify
        mock_repository.find_participant.assert_called_once_with(room_id, sample_guest.guest_id)
    ...
    async def test_handle_ready_toggle_creator(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest, mock_creator_participant):
    ...
    async def test_handle_ready_toggle_no_participant(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest):
    ...
    async def test_handle_ready_toggle_in_game(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest, mock_participant):
    ...
    async def test_handle_status_update_success(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest, mock_participant):
        ...
        # Execute
        await websocket_service.handle_status_update(message_data, mock_websocket, room_id, sample_guest)
    ...
    async def test_handle_status_update_invalid_status(self, websocket_service, mock_ws_manager, mock_websocket, sample_guest):
    ...
    async def test_handle_status_update_no_participant(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest):
    ...
    async def test_handle_word_chain_initialize_game(self, websocket_service, mock_websocket, sample_guest):
        """Test word chain game initialization"""
        # Setup
        message_data = {"action": "initialize_game"}
        room_id = 1
        
        with patch.object(websocket_service, '_handle_initialize_game', new_callable=AsyncMock) as mock_handler:
            # Execute
            await websocket_service.handle_word_chain_message(message_data, mock_websocket, room_id, sample_guest)
            
            # Verify
            mock_handler.assert_called_once_with(mock_websocket, room_id, sample_guest)
    
    async def test_handle_word_chain_start_game(self, websocket_service, mock_websocket, sample_guest):
        """Test word chain game start"""
        # Setup
        message_data = {"action": "start_game"}
        room_id = 1
        
        with patch.object(websocket_service, '_handle_start_game', new_callable=AsyncMock) as mock_handler:
            # Execute
            await websocket_service.handle_word_chain_message(message_data, mock_websocket, room_id, sample_guest)
            
            # Verify
            mock_handler.assert_called_once_with(message_data, mock_websocket, room_id, sample_guest)
    
    async def test_handle_word_chain_submit_word(self, websocket_service, mock_websocket, sample_guest):
        """Test word chain word submission"""
        # Setup
        message_data = {"action": "submit_word"}
        room_id = 1
        
        with patch.object(websocket_service, '_handle_submit_word', new_callable=AsyncMock) as mock_handler:
            # Execute
            await websocket_service.handle_word_chain_message(message_data, mock_websocket, room_id, sample_guest)
    ...
    async def test_handle_word_chain_end_game(self, websocket_service, mock_websocket, sample_guest):
        """Test word chain game end"""
        # Setup
        message_data = {"action": "end_game"}
        room_id = 1
        
        with patch.object(websocket_service, '_handle_end_game', new_callable=AsyncMock) as mock_handler:
            # Execute
            await websocket_service.handle_word_chain_message(message_data, mock_websocket, room_id, sample_guest)
    ...
    async def test_handle_initialize_game_not_creator(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest):
        ...
        # Execute
        await websocket_service._handle_initialize_game(mock_websocket, room_id, sample_guest)
    ...
    async def test_handle_start_game_not_creator(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest):
        ...
        # Execute
        await websocket_service._handle_start_game(message_data, mock_websocket, room_id, sample_guest)
    ...
    async def test_handle_submit_word_empty_word(self, websocket_service, mock_ws_manager, mock_websocket, sample_guest):
        ...
        # Execute
        await websocket_service._handle_submit_word(message_data, mock_websocket, room_id, sample_guest)
        
        # Verify
        mock_ws_manager.send_personal_message.assert_called_once_with(
            {"type": "error", "message": "단어를 입력해주세요."},
            mock_websocket
        )
    
    async def test_handle_submit_word_success(self, websocket_service, mock_ws_manager, mock_websocket, sample_guest):
        ...
        # Verify
        mock_ws_manager.submit_word.assert_called_once_with(room_id, "사과", sample_guest.guest_id)
        mock_ws_manager.broadcast_room_update.assert_called_once()
    ...
    async def test_handle_submit_word_failure(self, websocket_service, mock_ws_manager, mock_websocket, sample_guest):
    ...
    async def test_handle_end_game_not_creator(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest):
    ...
    async def test_handle_end_game_success(self, websocket_service, mock_repository, mock_ws_manager, mock_websocket, sample_guest):
    ...
    async def test_handle_host_change_notification(self, websocket_service, mock_ws_manager):
        ...
        # Execute
        await websocket_service.handle_host_change_notification(room_id, new_host_id, new_host_nickname)
    ...
    async def test_handle_participant_list_update(self, websocket_service, mock_repository, mock_ws_manager):
