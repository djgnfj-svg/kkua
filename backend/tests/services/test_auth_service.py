...
class TestAuthService:
    ...
    def auth_service(self, mock_guest_repo, mock_session_store):
        ...
        with patch(
            "services.auth_service.get_session_store", return_value=mock_session_store
        ):
    ...
    def test_login_with_new_nickname(
        self, auth_service, mock_guest_repo, mock_session_store, sample_guest
    ):
    ...
    def test_login_with_existing_nickname(
        self, auth_service, mock_guest_repo, mock_session_store, sample_guest
    ):
        ...
        mock_guest_repo.update_last_login.assert_called_once_with(
            sample_guest, "test_device"
        )
        mock_session_store.create_session.assert_called_once_with(1, "테스트사용자")
    ...
    def test_update_profile_success(
        ...
        mock_guest_repo.update_nickname.assert_called_once_with(
            sample_guest, "새닉네임"
        )
        mock_session_store.update_session.assert_called_once_with(
            "valid_token", nickname="새닉네임"
        )
    ...
    def test_set_auth_cookies(self, auth_service):
        ...
        with patch(
            "services.auth_service.CSRFTokenManager.get_csrf_token_for_response",
