...
class TestGameroomModel:
    def test_gameroom_creation(self, db_session: Session):
        ...
        # Create a gameroom
        room = Gameroom(title="Test Room", max_players=8, created_by=guest.guest_id)
        db_session.add(room)
        db_session.commit()
    ...
    def test_gameroom_default_values(self, db_session: Session):
        ...
        room = Gameroom(title="Default Test", created_by=guest.guest_id)
        db_session.add(room)
        db_session.commit()
    ...
    def test_gameroom_relationship(self, db_session: Session):
        ...
        # Create room
        room = Gameroom(title="Relationship Test", created_by=guest.guest_id)
        db_session.add(room)
        db_session.commit()
