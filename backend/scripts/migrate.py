...
def upgrade(revision="head"):
    ...
    return run_command(["alembic", "upgrade", revision], f"Upgrading database to {revision}")
...
def show_current():
    ...
    return run_command(["alembic", "current", "--verbose"], "Showing current database revision")
...
def stamp(revision):
    ...
    return run_command(["alembic", "stamp", revision], f"Stamping database with revision {revision}")
...
def main():
    ...
    if not os.path.exists("alembic/env.py") and args.command != "init":
        print("❌ Alembic is not initialized. Please run the init_alembic.py script first.")
        return False
