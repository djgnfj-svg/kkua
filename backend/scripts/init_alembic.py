...
def main():
    ...
    if not run_command(["python", "-c", "import alembic; print(f'Alembic version: {alembic.__version__}')"], 
    ...
    if os.path.exists("alembic/versions"):
        ...
        create_migration = input("Do you want to create a new migration? (y/N): ").lower().strip()
        
        if create_migration == 'y':
            # Create a new migration
            migration_message = input("Enter migration message (or press Enter for 'Update schema'): ").strip()
            if not migration_message:
                migration_message = "Update schema"
            
            return run_command(["alembic", "revision", "--autogenerate", "-m", migration_message],
    ...
    if run_command(["alembic", "revision", "--autogenerate", "-m", "Initial database schema"],
        ...
        print("3. To create new migrations: alembic revision --autogenerate -m 'description'")
