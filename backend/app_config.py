...
class Settings(BaseSettings):
    ...
    database_url: str = os.getenv(
        "DATABASE_URL", "postgresql://postgres:CHANGE_ME_IN_PRODUCTION@db:5432/mydb"
    )
    ...
    def cors_origins(self) -> List[str]:
        return [
            origin.strip() for origin in self._cors_origins.split(",") if origin.strip()
        ]
