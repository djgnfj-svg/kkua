                    ...
                    message.update(
                        {
                            ...
                            "score_breakdown_message": score_calculator.create_score_breakdown_message(
                                score_info
                            ),
                        }
                    )
...
async def get_game_state(room_id: int, guest: Guest = Depends(require_authentication)):
    """게임 상태 조회"""
    try:
        redis_game = await get_redis_game_service()
...
async def get_player_stats(
    room_id: int, guest: Guest = Depends(require_authentication)
):
    """플레이어 통계 조회"""
    try:
        redis_game = await get_redis_game_service()
...
async def end_game(room_id: int, guest: Guest = Depends(require_authentication)):
    """게임 종료 (방장만 가능)"""
    try:
        redis_game = await get_redis_game_service()
        game_state = await redis_game.get_game_state(room_id)
