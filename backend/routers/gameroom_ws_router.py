            ...
            try:
                ...
                if hasattr(message_service, "validate_and_process_message"):
                    try:
                        ...
                        logger.warning(
                            f"새로운 검증 시스템 실패, fallback 사용: {validation_error}"
                        )
                        success = False
                ...
                if not success:
                    ...
                    if message_type == "chat":
                        await message_service.handle_chat_message(
                            message_data, room_id, guest
                        )
                    elif message_type == "toggle_ready":
                        await message_service.handle_ready_toggle(
                            websocket, room_id, guest
                        )
                    elif message_type == "status_update":
                        await message_service.handle_status_update(
        ...
        if guest and room_id:
            try:
                ...
                for participant in participants:
                    guest_in_ws = any(
                        conn.get("guest_id") == participant.guest_id
                    ...
                    if not guest_in_ws:
                        ...
                        logger.info(
                            f"고아 참가자 정리: room_id={room_id}, guest_id={participant.guest_id}"
                        )
