...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        request.state.session_id = session_token[:8] if session_token != "anonymous" else "anonymous"
    ...
    def _should_audit(self, request: Request, response: Response) -> bool:
        ...
        if request.url.path.startswith('/gamerooms/') and request.method in ['POST', 'PUT', 'DELETE']:
            return True
